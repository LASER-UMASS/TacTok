...
if [[ ${NUM_TRAIN_STEPS} -ne ${EXPECTED_STEPS_TRAIN} ]] ; then
    echo "Wrong number of proof steps in $TT_DIR/TacTok/proof_steps/train/; expected ${EXPECTED_STEPS_TRAIN}, got ${NUM_TRAIN_STEPS}"
    exit 1
fi
...
if [[ ${NUM_VALID_STEPS} -ne ${EXPECTED_STEPS_VALID} ]] ; then
    echo "Wrong number of proof steps in $TT_DIR/TacTok/proof_steps/train/; expected ${EXPECTED_STEPS_VALID}, got ${NUM_VALID_STEPS}"
    exit 1
fi
